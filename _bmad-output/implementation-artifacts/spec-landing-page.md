...
- **Critical bug found via manual browser verification (`npm run dev` + Playwright), not by any of the 3 automated review layers**: `const resend = new Resend(process.env.RESEND_API_KEY)` was constructed at module scope in `lib/resend.ts`. The Resend SDK's constructor validates its argument and throws synchronously when it's missing — so simply *importing* `lib/resend.ts` crashed with `Error: Missing API key` whenever `RESEND_API_KEY` wasn't set, in any environment, before the `isProduction` gate ever ran. This broke the contact form's actual "happy path" and "Non-Production env" matrix rows in practice (visible as a 500 in the browser) despite every unit test passing, because all 3 tests that exercise this code path mock the `resend` package away and never hit the real constructor. *Fixed:* the client is now constructed lazily, inside the function, only after confirming `isProduction` and that `RESEND_API_KEY`/`RESEND_FROM_EMAIL`/`CONTACT_TO_EMAIL` are all present. Re-verified end-to-end in a real browser: happy path now shows the `role="status"` success message, and a validation-error-then-correct resubmission also completes successfully with the earlier answers preserved (see next note).
- **Second bug found the same way**: after a validation error, every field the visitor had already typed was wiped, forcing a full retype. Cause: React resets uncontrolled `<form>` fields after every action call (mirroring native form-reset behavior). *Fixed:* `ContactFormState`'s `validation`/`technical` cases now carry back the submitted `values`; each `<input>` receives them as `defaultValue`, and the `<select>` (whose `defaultValue` React only honors at mount, not on later prop changes) is synced imperatively via its ref inside the existing focus-management `useEffect`. Covered by a new assertion in `__tests__/actions.test.ts` (the validation-error and technical-error tests now check `result.values`); the `<select>`-specific redisplay quirk itself isn't unit-testable (it's a DOM/React-runtime interaction) and was confirmed by direct browser verification instead.
